class ParticipantsController < ApplicationController
  ...
  def join
	...
	respond_to do |format|
		if @participant.meetings << @meeting
			flash[:notice] = "Participation confirmed by user."
			format.html {redirect_to(@participant)}
			format.xml {head :ok}
		else
			flash[:notice] = "Failed to confirm participation by user."
			format.html {redirect_to(@participant)}
        	format.xml  { render :xml => @participant.errors, :status => :unprocessable_entity }
		end
	end
  ...
  def disjoin
	...
	respond_to do |format|
		if @participant.meetings.delete(@meeting)
			flash[:notice] = "Participant left the meeting."
			format.html {redirect_to(@participant)}
			format.xml {head :ok}
		else
			flash[:notice] = "Failed to left the meeting."
			format.html {redirect_to(@participant)}
        	format.xml  { render :xml => @participant.errors, :status => :unprocessable_entity }
		end
	end
